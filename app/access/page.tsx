      ...
      <main className="w-full flex flex-col">
        ...
        <section className="bg-[#1E2B22] w-full">
          <div className="max-w-5xl mx-auto px-6 md:px-8 pt-12 pb-10 md:pt-16 md:pb-14 flex flex-col gap-3">
            ...
            <h1 className="text-[24px] md:text-[36px] font-bold text-white leading-snug tracking-tight">アクセス・問い合わせ</h1>
            <p className="text-[13px] md:text-[15px] text-[#9AB5A4] leading-relaxed">三軒茶屋駅（東急田園都市線・世田谷線）徒歩3分</p>
          </div>
        </section>

        {/* Map + Info */}
        <section className="bg-[#F5F4F1] w-full">
          <div className="max-w-5xl mx-auto px-5 md:px-8 py-14 md:py-20 flex flex-col gap-6">

            <div className="flex flex-col md:flex-row gap-5">
              {/* Map */}
              <div className="h-52 md:h-auto md:flex-1 rounded-xl bg-[#D8E4D8] flex items-center justify-center min-h-[200px]">
                <p className="text-[13px] text-[#4A6E55] font-medium tracking-wide">地図（Google Maps）</p>
              </div>

              {/* Info */}
              <div className="bg-white rounded-xl p-5 md:p-6 flex flex-col gap-4 md:w-[340px] md:shrink-0">
                <h2 className="text-[14px] md:text-[15px] font-bold text-[#1A1918]">店舗情報</h2>
            ...
            {/* LINE */}
            <div id="contact" className="bg-[#06C755] rounded-xl p-5 md:p-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4 md:gap-8">
              <div className="flex flex-col gap-1">
                <p className="text-[10px] tracking-[0.15em] text-[#04A045] font-semibold">LINE RESERVATION</p>
                <p className="text-[16px] md:text-[20px] font-bold text-white">LINEでかんたん予約</p>
                <p className="text-[13px] text-white/80 leading-relaxed mt-1">
                  友だち追加後、ご希望の日時・メニューをメッセージください。24時間受付中です。
                </p>
              </div>
              <a href="#" className="shrink-0 block text-center py-[13px] px-8 rounded-lg bg-white text-[#06C755] font-bold text-[14px]">
                LINE 友だち追加
              </a>
            </div>

            {/* Contact Form */}
            <div className="bg-white rounded-xl p-5 md:p-8 flex flex-col gap-5">
              <div className="flex flex-col gap-1">
                <p className="text-[10px] tracking-[0.15em] text-[#3D8A5A] font-semibold">CONTACT FORM</p>
                <h2 className="text-[15px] md:text-[17px] font-bold text-[#1A1918]">お問い合わせ・ご予約フォーム</h2>
              </div>
              ...
              {submitted ? (
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                  ...
                  <button type="submit" className="w-full md:w-auto md:self-start md:px-10 py-[14px] rounded-lg bg-[#3D8A5A] text-white font-bold text-[14px] hover:bg-[#2E6B44] transition-colors">
                    送信する
                  </button>
